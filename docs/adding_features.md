...
5. **Register dependency** in `backend/core/dependencies.py`
   ```python
...
6. **Create route** in `api/routes/v1/`
   ...
   from backend.api.deps import CurrentUser, NotificationSvc
...
7. **Register router** in `api/routes/v1/__init__.py`
   ```python
   from backend.api.routes.v1 import notifications
...
Commands are auto-discovered from `backend/commands/`.

```python
# backend/commands/my_command.py
from backend.commands import command, success, error
import click
