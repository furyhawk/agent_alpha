...
Contains the main RESEARCH_PROMPT, subagent instructions,
and the quick-reference skill content.
"""
...
Search your organization's uploaded documents using the RAG tools:
- `rag_search(query)` — search across all uploaded documents
- `rag_search_by_document(query, filename)` — search within a specific document
- `rag_list_collections()` — list available document collections

Use RAG when the user asks about their uploaded files, documents, or internal knowledge \
base. Combine RAG results with web search for comprehensive answers.
