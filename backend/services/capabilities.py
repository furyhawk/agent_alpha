"""Capabilities for the research agent.

Contains middleware that provides:
- AuditCapability: tracks tool usage stats
- PermissionCapability: blocks access to sensitive paths
- RateLimitRetryCapability: auto-retries on rate-limit errors
- ForgiveWriteTodosCapability: handles malformed tool calls from weak LLMs
"""
...
@dataclass
class ForgiveWriteTodosCapability(AbstractCapability[DeepAgentDeps]):
    """Capability that forgives empty ``write_todos({})`` from weak LLMs.

    Some local LLMs with weak function calling produce empty ``todos`` when
    writing tasks. This capability catches the ``ValidationError`` and substitutes
    an empty list so the agent doesn't crash.
    """
    ...
    async def on_tool_validate_error(
        ...
        *,
        ...
        if "todos" in err_str and write_todos_matches(tool_def.name):
            logger.warning(
                f"ForgiveWriteTodosCapability: caught empty todos error for "
                f"{tool_def.name}: {error}. Substituting empty list."
            )
            return {"todos": []}
        raise error


def write_todos_matches(tool_name: str) -> bool:
    """Check if tool name looks like a write_todos variant."""
    return tool_name in {"write_todos", "update_todos", "set_todos"}
